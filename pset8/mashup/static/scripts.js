...
{
    // Instantiate marker for every place in the US map
    ...
    // Listen for clicks on markers within the map
    marker.addListener('click', function() {
        // Show info window if marker is clicked
        showInfo(marker);

        // Get articles for particular place
        $.getJSON("/articles", {geo: place.postal_code}, function(data, textStatus, jqXHR) {
            let content = '<ul>';

            // Build list of links to those articles
    ...
    // Remember marker so we can later on remove them
    markers.push(marker);
}
